#! /bin/sh
export MODEL_NAME=${LANG_ISO}_vectors_web_${LANG_SIZE}-1.1.0

python -m app ${LANG_ISO} ${LANG_NAME} ${TRAINING_TYPE} ${OVERWRITE_LANG} ${LANG_SIZE} ${PRUNE_VECTORS} ${DOWNLOAD_SOURCE}
...
aws s3 cp dist/${MODEL_NAME}.tar.gz s3://${AWS_BUCKET_NAME}/${MODEL_NAME}.tar.gz

# Commenting this out because we're not forking the original Rasa repo anymore
